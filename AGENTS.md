...
For every release, run native builds/tests for both architectures and the signing, notarization, archive-signature, download, and Homebrew checks. Once a public baseline exists, require the real previous-version Sparkle installation/relaunch gate. For app-binary changes, run `scripts/release/lifecycle_smoke.py` against the permitted development app; add `--controls` for fixed-volume media-key/HUD coverage. These results do not replace physical observations.
